...
export async function POST(request: Request) {
  ...
  switch (event.type) {
    ...
    case "customer.subscription.updated": {
      ...
      if (!userId) {
        console.log("No userId in subscription metadata, fetching from customer...")
        try {
      ...
      if (userId) {
        ...
        if (subscription.status === "active") {
          creditsAvailable = getCreditLimitByPriceId(subscription.items.data[0].price.id)
          console.log("Credits calculated:", creditsAvailable, "for price:", subscription.items.data[0].price.id)
        }
        ...
        const { data, error } = await supabaseServer.from("users").upsert(updateData, {
          onConflict: "id"
        })
