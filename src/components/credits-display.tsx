...
export function CreditsDisplay() {
  ...
  useEffect(() => {
    const fetchCredits = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          console.error("CreditsDisplay: Credits API failed with status:", response.status)
  ...
  if (loading) {
    return (
      <div className="flex items-center space-x-2 px-3 py-1.5 rounded-full bg-muted animate-pulse">
